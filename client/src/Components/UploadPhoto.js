        ...
        <ScrollView contentContainerStyle={styles.scrollViewContainer}>
          {
            images.map((image, index) => {
              return (<TouchableWithoutFeedback key={index} onPress={() => this.updateSelectedImage(image, index)}>
                <Image source={{
      ...
      </View>
      {
        selectedImageIndex || selectedImageIndex === 0
          ? (<View style={{
            ...
            <Icon onPress={() => this.props.navigation.goBack()} raised="raised" reverse="reverse" name='check' size={34} containerStyle={{
                width: 50,
                height: 50
